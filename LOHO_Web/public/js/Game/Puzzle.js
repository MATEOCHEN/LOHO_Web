(function() {
    function puzzle() {
        ...
        this.failCover=document.getElementById("fail");
    ...
    puzzle.prototype.eventBind=function(){
        ...
        //重置&再来一次
        this.btn.reset.onclick =function() {
        ...
        this.btn.agian.onclick=function() {
            _this.nowOrder=roa([0, 1, 2, 3, 4, 5, 6, 7, 8]);
            _this.step=10;
            _this.blockInit();
            _this.eventBind();
            _this.failCover.style.display = "none";
            _this.block[8].style.display = "none";
        }
        //显示隐藏数字
        this.btn.showNum.onclick = function() {
            if(_this.isShowOrder=='顯示序號'){
                Array.prototype.forEach.call(_this.order,function(ele,i){
                    ele.style.display = "block";
                })
                _this.isShowOrder='隐藏序號';
            }else{
                Array.prototype.forEach.call(_this.order,function(ele,i){
                    ele.style.display = "none";
                })
                _this.isShowOrder='顯示序號';
            }
    ...
    puzzle.prototype.failShow=function(){
        ...
        this.stepCount.innerHTML = this.step;
    }
    //数组随机排序
    function roa(ar) {
        var arr = ar;
